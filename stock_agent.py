...
# Optional barcode integration. carico/scarico work with or without it; only the
# label-generation and scan-resolution extras need these libraries.
try:
...
def add_carico(data, validate=True, allow_duplicate=False):
    ...
    # col Y: maker barcode scanned at intake ("link" step) so a later scan of
    # that same code maps straight back to this row.
    cod_forn = data.get("codice_fornitore")
    if cod_forn:
        ws.cell(r, COL["CODICE_A_BARRE"]).value = str(cod_forn).strip()
    ...
    # optionally generate a BCW QR+Code128 label PDF for this item
    if data.get("genera_etichetta"):
        if not _BARCODE_OK:
            result["etichetta_warning"] = (
                "Librerie barcode non disponibili; etichetta non generata.")
        else:
            _bc.EXCEL_FILE = EXCEL_FILE  # operate on the same workbook
            gen = _bc.cmd_genera({
...
def add_scarico(data):
    ...
    # resolve by scanned barcode first, if a raw code was passed
    row = None
